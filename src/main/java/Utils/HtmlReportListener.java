  ...
  implements IInvokedMethodListener, ITestListener, ISuiteListener, IExecutionListener
{
  ...
  static  {
    ...
    if (reportProperties.getProperty("buildname") != null && 
      !reportProperties.getProperty("buildname").equals("")) {
      BUILDNAME = reportProperties.getProperty("buildname");
    } else {
  ...
  public static String getScreenshotPath() { return screenshotPath; }
  ...
  public static boolean getParallel() { return runParallel; }
    ...
    if (driver != null) {
      if (CAPTURE_SCREENSHOT) {
        ...
        if (browser == null || browser.equalsIgnoreCase("")) {
          ...
          imageFile = screenshotPath + File.separator + sTestMethodName + "-" + String.valueOf(invocationCount) + "-" + Utils.getCurrentDateTime("ddMMMyy_hhmmss") + ".png";
        }
        else {
          
          imageFile = screenshotPath + File.separator + sTestMethodName + "-" + browser.replaceAll("\\*", "") + "-" + String.valueOf(invocationCount) + "-" + Utils.getCurrentDateTime("ddMMMyy_hhmmss") + ".png";
        } 
        ...
        Reporter.log("<a href='" + Utils.getRelativePath(imageFile, resultDir + File.separator + "html") + "'><img src='" + 
            Utils.getRelativePath(imageFile, resultDir + File.separator + "html") + "' height='50' width='50'/><br></a>");
      } else {
    ...
    if (GEN_ADVANCED_REPORT) {
      synchronized (this) {
        ...
        if (!methodsDependOn.toString().equalsIgnoreCase("")) {
          MainReporting.addResultInfoNode(iTestResult, "Test has Dependent methods: " + methodsDependOn
              .substring(0, methodsDependOn.length() - 2), 4);
        }
        ...
        if (!groupsDependedOn.toString().equalsIgnoreCase("")) {
          MainReporting.addResultInfoNode(iTestResult, "Test has Dependent groups: " + groupsDependedOn
              .substring(0, groupsDependedOn.length() - 2), 4);
        }
    ...
    if (method.isTestMethod()) {
      ...
      if (verificationFailures.size() > 0) {
        ...
        if (size == 1) {
          ...
          StringBuffer failureMessage = (new StringBuffer("Multiple asserts failed (")).append(size).append("):\n");
  ...
  private void removeIncorrectlyFailedTests(ITestContext test) {
    ...
    if ((countOfAllResults == countOfPassedResults || countOfAllResults == countOfFailedResults) && countOffailedwithSuccessResults != 0)
    {
  ...
  public void onStart(ISuite arg0) {
    ...
    if (!arg0.getParallel().equalsIgnoreCase("none") && EnvParameters.EXECUTION_ENV.equals("local")) {
      if (arg0.getParallel().equalsIgnoreCase("methods")) {
        throw new ItJumpStartReportException("parallel=\"methods\" at suite level in testng XML is not supported as test methods are not thread safe");
      }
      
      runParallel = true;
      LoggerUtil.log("Video Recording will be disabled as test(s) will be run in parallel locally", Level.WARN);
    } 
      ...
      for (XmlTest test : arg0.getXmlSuite().getTests()) {
        if (test.getParallel().isParallel()) {
          if (test.getParallel() == XmlSuite.ParallelMode.METHODS) {
            throw new ItJumpStartReportException("parallel=\"methods\" at test level in testng XML is not supported as test methods are not thread safe");
          }
  ...
  public void onExecutionStart() {
    ...
    if (ARCHIVE_REPORT) {
      ...
      try {
        ...
        archivePath = System.getProperty("user.home") + File.separator + "Hawk Automation Test Report" + File.separator + TESTTYPE + " Results" + File.separator + strCurrYear + File.separator + theMonth(Integer.parseInt(strCurrMonth)) + File.separator;
        ...
        outdir = archivePath;
        LoggerUtil.updateLog4jConfiguration(archivePath + File.separator + "logs" + File.separator + "report.log");
